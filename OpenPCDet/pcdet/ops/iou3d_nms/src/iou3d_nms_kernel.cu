...
__device__ inline float iou_bev_3D(const float *box_a, const float *box_b){
    // params box_a: [x, y, z, dx, dy, dz, heading]
    // params box_b: [x, y, z, dx, dy, dz, heading]
    float sa = box_a[3] * box_a[4] * box_a[5];
    float sb = box_b[3] * box_b[4] * box_b[5];
    float top = fmaxf(box_a[2] - box_a[5] / 2, box_b[2] - box_b[5] / 2);
    float bottom = fminf(box_a[2] + box_a[5] / 2, box_b[2] + box_b[5] / 2);
    float height = fmaxf(bottom - top, 0.f);
    float s_overlap = box_overlap(box_a, box_b) * height;
    return s_overlap / fmaxf(sa + sb - s_overlap, EPS);
}
...
__global__ void nms_kernel(const int boxes_num, const float nms_overlap_thresh,
    ...
    if (threadIdx.x < row_size) {
        ...
        for (i = start; i < col_size; i++) {
            if (iou_bev_3D(cur_box, block_boxes + i * 7) > nms_overlap_thresh){
                t |= 1ULL << i;
            }
        }
